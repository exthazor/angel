...
export default class BirthdayManager{
  ...
  async WishBirthday(birthdays: Birthdays[], client: BotClient) {
    ...
    let textStrings:string[] = ["Come on everyone! Let's wish a very happy birthday to ", "Alright guys instead of spewing drama all the time why don't you wish a very happy birthday to ", "Nico is an asshole he forgets everyone's birthdays because he is busy idolising fascism but you guys should definitely wish a happy birthday to  ", "Entropy is so dumb that he forgets everyone's birthdays but I remember them all because I am so smart hehe. So c'mon everyone let's wish a veryy happy birthday to  ", "Roses are red, violets are Blue, there's no one as cute and fun as you! Happy Birthday to ", "Berry might be a master manipulator but she could not succeed in making me not remind everyone to wish a very happy birthday to ", "Hey cuties!! Let's wish a very happy birthday to  "]

    let randomNumber = Math.floor(Math.random() * textStrings.length)
            
            BirthdayManager.arr.push(randomNumber)

            if(( BirthdayManager.arr.length > 1) && (BirthdayManager.arr[BirthdayManager.arr.length-1] == BirthdayManager.arr[BirthdayManager.arr.length-2])){
